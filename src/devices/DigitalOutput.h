...
class DigitalOutput : public BaseDevice
{
public:
    DigitalOutput(uint32_t pin1  = INVALID_PIN, uint32_t mode1  = INVALID_MODE, uint32_t pin2  = INVALID_PIN, uint32_t mode2  = INVALID_MODE,
                  ...
                  uint32_t pin15 = INVALID_PIN, uint32_t mode15 = INVALID_MODE, uint32_t pin16 = INVALID_PIN, uint32_t mode16 = INVALID_MODE)
    {
        ...
        addHoldingRegister("Digital outputs bitmask 0-15");
    ...
    void initialize() override
    {
        for (uint8_t i = 0; i < 16; i++)
        {
            if (_pins[i] != INVALID_PIN && _mods[i] != INVALID_MODE)
            {
                ...
                digitalWrite(getPin(_pins[i]), LOW);
            }
        }
    }
    ...
    {
        uint16_t mask = holdingRegisterRead(0);
        for (uint8_t i = 0; i < 16; i++)
        {
            if (_pins[i] != INVALID_PIN && _mods[i] != INVALID_MODE)
            {
                digitalWrite(getPin(_pins[i]), (mask >> i) & 1);
            }
        }
    }
